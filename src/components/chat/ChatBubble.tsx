    ...
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div
        className={`max-w-[85%] rounded-3xl px-4 py-3 text-sm leading-relaxed shadow-sm ${
          isUser ? 'bg-sky-500 text-white shadow-lg shadow-sky-500/15' : 'bg-[#172337] text-slate-100'
        }`}
      >
        <p>{message.text}</p>
        {message.parsedPayload && (
          <div className="mt-3 space-y-2 rounded-2xl bg-sky-950/35 p-3 text-xs text-slate-200">
