import type { Client, Pedido, PedidoEstado, Proveedor } from './types';
...
export type PedidosQueryAction = {
...
const resolveEstado = (action: PedidosQueryAction): PedidoEstado | null => {
...
const estadoLabel = (estado: PedidoEstado | null, count: number) => {
...
const resolveClientName = (pedido: Pedido, clientsById: Record<string, Client>) =>
  clientsById[pedido.clienteId]?.name?.trim() || 'Sin cliente';

const resolveProveedorName = (pedido: Pedido, proveedoresById: Record<string, Proveedor>) =>
  (pedido.proveedorId ? proveedoresById[pedido.proveedorId]?.name?.trim() : '') || '';

export const answerPedidosQuery = (
  pedidos: Pedido[],
  clients: Client[],
  proveedores: Proveedor[],
  action: PedidosQueryAction = {},
  ...
  const groups = new Map<string, { title: string; items: Pedido[] }>();
